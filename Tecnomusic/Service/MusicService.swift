...
final class MusicService {
    ...
    func startPlaying(collection: MusicCollection, at music: Music? = nil) {
        ...
        var musicsToPlay = collection.musics
        if let firstMusic = music, let index = collection.musics.firstIndex(of: firstMusic) {
            musicsToPlay = Array(collection.musics[index...])
        }
        
        queue = Queue(
            nowPlaying: musicsToPlay.first,
            collection: collection,
            nextInCollection: Array(musicsToPlay.dropFirst()),
